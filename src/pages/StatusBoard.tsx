...
const discussionLog: DiscussionItem[] = [
  ...
  },
  {
    ...
    refNo: "#5",
    content:
      "비즈니스센터 > 브랜드 프로필 재직인증 선택사항으로 변경\n• 멤버십센터에서는 재직인증 없어도 운영상 문제 없음\n→ 재직인증 필드를 선택사항으로 변경, 브랜드 프로필 생성 절차 수정 재기획 필요",
    action: "브랜드 프로필 생성 플로우 재기획. 재직인증 필드 선택사항 처리 방식 UX 정의 및 비즈니스센터 FRD 업데이트",
    owner: "",
    status: "진행",
  },
  {
    ...
    action: "정산월 최대 1년 / 건수 최대 3개월 기준으로 우선 반영 예정. DateRangeFilter 컴포넌트 및 각 페이지 필터 스펙 업데이트",
    owner: "",
    status: "진행",
  },
  {
    ...
    refNo: "#7",
    content:
      "③ 권한별 기능 제한 — 엑셀/공문 다운로드, 항목 추가·삭제 접근 권한\n[의견] 공문 다운로드 및 적립·사용 수수료율 컬럼 조회는\n\"본사 관리자\" · \"구성원\" 역할에게만 노출.\n\n[추가] URL path에 활성 탭 포함 (예: /settlement/by-store)\n① 운영 문의 대응 시 사용자가 보고 있던 화면을 딥링크로 즉시 공유\n② 탭별로 권한 차등 필요 → 라우트 단위로 권한 제어",
    action: "역할별 기능 노출 매트릭스 확정 필요. 탭별 URL path 설계 및 라우트 단위 권한 제어 방식 결정. FRD 권한 정책(FR-AD03) 연계 업데이트",
    owner: "",
    status: "대기",
  },
  {
  ...
  },
  {
    ...
    refNo: "#9",
    content:
      "① 계정 잠금·탈퇴 정책 — GMS 정책 준용 vs CJ ONE 계정 정책 준용\n현행 GMS: 60일 미사용 시 잠금 / 90일 미사용 시 자동 탈퇴\n→ GMS 정책 대신 CJ ONE 계정 정책 준용(비즈니스센터와 동일)으로 결정",
    action: "CJ ONE 계정 정책 기준으로 파트너센터 적용 범위 확정. 비즈니스센터 FRD 해당 조항 참조하여 FRD 반영",
    owner: "",
    status: "완료",
  },
  {
  ...
  },
  {
    ...
    refNo: "#11",
    content:
      "대량 엑셀 다운로드 처리 방식 (최대 300만 건 수준)\n• 비동기 방식으로 다운로드 처리\n• 조회 조건 전체 데이터 다운로드(pagination 무시) → 개발 내부 논의 필요\n• 전일자 대사파일 생성 방안 검토: 한 달치 기준 다운로드 허용",
    action: "1차 개발PM과 논의 완료. 전체 데이터 다운로드(pagination 무시) 구현 방식 개발 내부 추가 논의 필요",
    owner: "",
    status: "진행",
  },
  {
  ...
  },
  {
    ...
    refNo: "#14",
    content:
      "1:1 문의 의견 접수 시 멤버십센터 어드민 관리자 메일 발송 스펙 — 리뷰 필요\n\n[발송 조건]\n• 의견 저장 성공 직후 발송\n• 작성 취소 / 닫기 / 저장 실패 시 미발송\n\n[메일 내용]\n• 제목: [CJ ONE 멤버십센터] 의견이 접수되었습니다\n• 본문: 일시 / 발신자 ID(마스킹 처리) / 의견 전문",
    action: "발송 대상 범위(수신자 설정 방식), 마스킹 규칙 상세, 메일 템플릿 디자인 리뷰 필요",
    owner: "",
    status: "대기",
  },
  {
    ...
    type: "반영 완료",
    refNo: "#15",
    content: "포인트 유형 드랍다운 리스트 확정 완료\n→ 조회 필터 내 '포인트 유형' 선택지 항목 및 표시 순서 확정",
    action: "포인트 유형 목록 확정 완료. 필터 컴포넌트 및 목업 데이터 반영",
    owner: "",
    status: "완료",
  },
  {
    ...
    type: "논의 필요",
    refNo: "#17",
    content: "적립/사용 수수료 정산 공문 사용 여부 확인 중\n→ 기존 GMS에서 발행하던 정산 공문을 파트너센터에서도 동일하게 제공할지 미결",
    action: "내부 검토 및 유관 부서 확인 후 공문 제공 범위·형식 결정 필요",
    owner: "",
    status: "진행",
  },
  {
    ...
    refNo: "#16",
    content:
      "현대카드 탭 > 발행 리워드 내역 + 소멸 내역 화면 병합 여부\n→ 올리브영 현대카드 제휴사 전용 탭 내 두 화면을 하나로 통합할지 분리 유지할지 미결\n→ 올리브영 정산 담당자 의견 확인 필요",
    action: "올리브영 정산 담당자 의견 수렴 후 화면 구성 방향 확정. HyundaiCardUseHistory 기획서 업데이트",
    owner: "",
    status: "대기",
  },
];
